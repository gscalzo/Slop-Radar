...
That left long posts, which are the slop heartland, half-analysed until the user
expanded them by hand, one by one. The requested middle ground: a command that expands
...
- **Expansion runs only from an explicit user action**: the `expand-truncated`
  ...
  truncated items exist. The extension never expands on its own: not on scan, not on
  scroll, not on a timer.
...
- Expansion covers items currently in the DOM. Newly scrolled-in clamped posts need
  another invocation, and the floating button's count makes that visible.
