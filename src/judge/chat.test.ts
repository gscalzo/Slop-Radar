...
describe("chatCompletion", () => {
  ...
  // Reasoning models reject any temperature but their default, so no caller
  // may send one. This is the regression guard for that.
  it("sends exactly the body it was given, with no sampling parameters added", async () => {
    ...
    await chatCompletion(ENDPOINT, { model: "o3-mini", messages: [] }, fetchFn, fail);

    expect(seen[0]).toEqual({ model: "o3-mini", messages: [] });
  });
});
