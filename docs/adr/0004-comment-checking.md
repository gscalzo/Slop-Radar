...
the feed, and increasingly AI-drafted, but they differ in two ways that matter: they
are numerous, and they are usually short. A density score (weight per 100 words)
...
- The abstain floor is **20 words for comments** against 40 for posts. Comments are
  short by nature, so demanding post-length text would exempt almost all of them.
- **Abstaining comments get no decoration at all**, no border and no badge. Posts
  keep their grey "n/a" badge; comments only surface when there is something to say.
...
- Sub-20-word generic praise ("Great insight, thanks for sharing! 🙌") is invisible
  ...
  than pattern density, which is out of scope for now.
- Even a 20-word floor makes comment verdicts noisier than post verdicts. The
  compact badge and the no-abstain-clutter rule mitigate that without fixing it.
